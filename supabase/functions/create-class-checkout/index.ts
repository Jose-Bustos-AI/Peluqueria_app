...
serve(async (req) => {
  ...
  try {
    ...
    // Load booking with class and user data
    const { data: booking, error: bookingError } = await supabaseAdmin
      ...
      .select(`
        ...
        type,
        payment_status,
        classes (id, name, price, currency, capacity),
    ...
    // Validate booking is type='class'
    if (booking.type !== 'class') {
      console.error('[Stripe] Invalid booking type:', booking.type)
      return new Response(
        JSON.stringify({ success: false, error: 'Invalid booking type - must be class' }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 400 
        }
    ...
    if (!booking.class_id) {
      console.error('[Stripe] missing class_id on booking')
      return new Response(
    ...
    if (!classData || !user) {
      console.error('[Stripe] Missing class or user data')
      return new Response(
    ...
    // Validate class_id exists
    if (!booking.start_at) {
      console.error('[Stripe] Missing start_at on booking')
      return new Response(
        JSON.stringify({ success: false, error: 'Missing start_at' }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 400 
        }
    ...
    // Check if there's a specific session capacity, otherwise use class capacity
    ...
    const capacity = sessionData?.capacity || classData.capacity
    ...
    console.log('[Stripe] Class capacity validation:', { capacity, currentCount })

    if ((currentCount || 0) >= capacity) {
      console.error('[Stripe] Class is at capacity')
      return new Response(
        JSON.stringify({ success: false, error: 'Class is full' }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 400 
        }
    ...
    const base_price = parseFloat(classData.price.toString())
    ...
    const currency = classData.currency || settingsObj['payments.currency_default'] || 'EUR'
    ...
    console.log('[Stripe] create class checkout booking=', booking_id, 'amount=', amount_cents, 'currency=', currency)
    ...
    const stripeResponse = await fetch('https://api.stripe.com/v1/checkout/sessions', {
      ...
      body: new URLSearchParams({
        ...
        'line_items[0][price_data][product_data][name]': classData.name || 'Clase',
    ...
    console.error('[Stripe] Error in create-class-checkout:', error)
